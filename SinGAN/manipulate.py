...
def generate_gif(Gs,Zs,reals,NoiseAmp,opt,alpha=0.1,beta=0.9,start_scale=2,fps=10):
    ...
    for G,Z_opt,noise_amp,real in zip(Gs,Zs,NoiseAmp,reals):
        ...
        if count == 0:
            ...
            z_prev1 = 0.95*Z_opt +0.05*functions.generate_noise([opt.nc_z,nzh,nzw], device=opt.device)
            z_prev2 = Z_opt
        ...
        for i in range(0,100,1):
            if count == 0:
                ...
                diff_curr = beta*(z_prev1-z_prev2)+(1-beta)*(functions.generate_noise([opt.nc_z,nzh,nzw], device=opt.device))
...
def SinGAN_generate(Gs,Zs,reals,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=50):
    ...
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
        ...
        for i in range(0,num_samples,1):
            if n == 0:
                z_curr = functions.generate_noise([1,nzh,nzw], device=opt.device)
                z_curr = z_curr.expand(1,3,z_curr.shape[2],z_curr.shape[3])
                z_curr = m(z_curr)
            else:
                z_curr = functions.generate_noise([opt.nc_z,nzh,nzw], device=opt.device)
                z_curr = m(z_curr)
